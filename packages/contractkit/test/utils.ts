...
import { getStaticNodesAsync } from '../src/static-node-utils'
import { getWeb3, getWeb3WithSigningAbility } from '../src/web3-utils'
...
export async function getIpAddressOfTxnNode(networkName: string): Promise<string> {
  if (cachedIpAddress === null) {
    ...
    const staticNodes: string = JSON.parse(await getStaticNodesAsync(networkName))
...
export async function getWeb3ForTesting(): Promise<Web3> {
  ...
  return getWeb3('http', ipAddress, HTTP_PROVIDER_PORT)
}

export async function getWeb3WithSigningAbilityForTesting(privateKey: string): Promise<Web3> {
  const ipAddress: string = await getIpAddressOfTxnNode(NETWORK_NAME)
  return getWeb3WithSigningAbility('http', ipAddress, HTTP_PROVIDER_PORT, privateKey)
}
