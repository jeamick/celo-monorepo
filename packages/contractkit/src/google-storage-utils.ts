...
function constructGoogleStorageUrl(bucketName: string, fileName: string): string {
  return `https://www.googleapis.com/storage/v1/b/${bucketName}/o/${fileName}?alt=media`
}

export async function fetchFileFromGoogleStorage(
  bucketName: string,
  fileName: string
): Promise<string> {
  Logger.debug(
  ...
  const url = constructGoogleStorageUrl(bucketName, fileName)
