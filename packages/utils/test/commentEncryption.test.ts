...
describe('Comment Encryption', () => {
  ...
  describe('Encrypt', () => {
    it('should encrypt message without error', () => {
      const { comment: ciphertext, encrypted } = encryptComment(
        comment,
        recip.getPublic(true),
        self.getPublic(true)
      )
  ...
  describe('roundtrip', () => {
    it('should return the same plaintext as sender', () => {
      const { comment: encryptedData } = encryptComment(
        comment,
        recip.getPublic(true),
        self.getPublic(true)
      )
      const { comment: plaintext, encrypted } = decryptComment(
        encryptedData,
        self.getPrivate() as Buffer,
        true
      )
  ...
  describe('decrypt', () => {
    it('should return comment if comment is not encrypted', () => {
      const { comment: decrypted, encrypted } = decryptComment(
        comment,
        self.getPrivate() as Buffer,
        true
      )
...
describe('decompressPublicKey', () => {
  it('should work with compressed input', () => {
    ...
    const publicKeyCompressed = privateKey.getPublic(true)
