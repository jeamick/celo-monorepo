...
} from '@celo/contractkit/lib/src/genesis-block-utils'
import { getStaticNodesAsync } from '@celo/contractkit/lib/src/static-node-utils'
...
async function createNewGeth(): Promise<typeof RNGeth> {
  ...
  const networkID: number = getChainIdFromGenesis(genesis)
...
async function ensureStaticNodesInitialized(): Promise<boolean> {
  ...
  if (await staticNodesAlreadyInitialized(nodeDir)) {
    ...
    try {
      enodes = await getStaticNodesAsync(currentNetworkName)
    } catch (error) {
      Logger.error(
...
async function ensureGenesisBlockWritten(): Promise<boolean> {
  ...
  if (await genesisBlockAlreadyWritten(nodeDir)) {
    ...
    try {
      genesisBlock = await getGenesisBlockAsync(currentNetworkName)
    } catch (error) {
